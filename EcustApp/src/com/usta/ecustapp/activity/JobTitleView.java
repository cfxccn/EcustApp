...
import com.usta.ecustapp.service.JobService;
	...
	private void initlistview() {
		...
		for (int i = 0; i < jobsTitilesJsonArray.length(); i++) {
			...
			map.put("textView_Job_title", jobJsonObject.optString("infotitile")
					.trim());
			map.put("textView_Job_treatment",
					jobJsonObject.optString("jobtreatment").trim());
			map.put("textView_Job_releasetime",
					jobJsonObject.optString("releasetime").trim());
			if (jobJsonObject.optString("qinban_cert").trim()
					.equalsIgnoreCase("是")) {
		...
		SimpleAdapter listItemAdapter = new SimpleAdapter(
				...
				// 动态数组与ImageItem对应的子项
				new String[] { "textView_Job_title", "textView_Job_treatment",
						"textView_Job_releasetime", "qb_cert", "textView_jobid" },
