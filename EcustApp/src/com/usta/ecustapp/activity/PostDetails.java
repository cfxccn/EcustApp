...
public class PostDetails extends ActionBarActivity {
		...
		btnPostBack.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View arg0) {
				...
				if (useremail.equals("null")) {
					ToastUtil.showToastShort(getParent(), "请先至 设置-账号管理 登录");
					// toast1.show();
				} else {
	...
	private Handler newPostBackSuccess = new Handler() {
		...
		public void handleMessage(Message msg) {
			...
			ToastUtil.showToastShort(getParent(), "发送成功");
	...
	private Handler newPostBackFailureNet = new Handler() {
		...
		public void handleMessage(Message msg) {
			super.handleMessage(msg);
			ToastUtil.showToastShort(getParent(), "发送失败 请检查网络");
			// toast4.show();
		}
	};
	private Handler newPostBackFailure = new Handler() {
		...
		public void handleMessage(Message msg) {
			super.handleMessage(msg);
			ToastUtil.showToastShort(getParent(), "请先至 设置-账号管理 重新登录");
			// toast3.show();
		}
	};
	...
	private void newPostBackViaNewThread() {
		...
		new Thread(new Runnable() {
			@Override
			public void run() {
				int i = postService.newPostBack(postid, text, useremail,
						userkey, anony);
				if (i == 1) {
	...
	protected void initlistview() {
		...
		textViewPBTime.setText(firstJsonObject.optString("date")
				.replace("T", " ").substring(0, 19).trim());
		...
		if (firstJsonObject.optString("anonymity").equals("1")) {
			...
			textViewPBUser.setText(firstJsonObject.optJSONObject("user")
					.optString("username"));
		...
		for (int i = 0; i < postbacksJsonArray.length(); i++) {
			...
			map.put("textViewPBText", postbackJsonObject.optString("text")
					.trim());
			map.put("textViewPBTime", postbackJsonObject.optString("date")
					.replace("T", " ").substring(0, 19).trim());
			if (postbackJsonObject.optInt("anonymity") == 1) {
				map.put("textViewPBUser", "匿名");
			} else {
				map.put("textViewPBUser",
						postbackJsonObject.optJSONObject("user")
								.optString("username").trim());
			}
			listItem.add(map);
		}
		// TODO Auto-generated method stub
		SimpleAdapter listItemAdapter = new SimpleAdapter(this, listItem,// 数据源
				...
				// 动态数组与ImageItem对应的子项
				new String[] { "textViewPBText", "textViewPBTime",
						"textViewPBUser" },
				// ImageItem的XML文件里面的一个ImageView,两个TextView ID
				new int[] { R.id.textViewPBText, R.id.textViewPBTime,
						R.id.textViewPBUser });
